...
/// @notice Full local deployment: v4 PoolManager, swap router, hook, seeded box.
contract Deploy is Script {
    ...
    function run() external {
        ...
        bytes memory ctorArgs =
            abi.encode(IPoolManager(address(pm)), packs, uint256(11_500), uint256(9_000), msg.sender);
        (address hookAddress, bytes32 salt) =
            HookMiner.find(CREATE2_FACTORY, flags, type(BoxHook).creationCode, ctorArgs);
        hook = new BoxHook{salt: salt}(IPoolManager(address(pm)), packs, 11_500, 9_000, msg.sender);
        ...
        PoolKey memory key =
            PoolKey(CurrencyLibrary.ADDRESS_ZERO, Currency.wrap(address(packs)), 0, 60, IHooks(hook));
